"""evo_probe.py — Evo-1 (StripedHyena, 7B) end-to-end PCA + SAE probing on DNA.

Forked from evo2_probe.py because Evo-2 7B (arcinstitute/evo2_7b) ships as a
raw .pt checkpoint that needs Arc Institute's `vortex` / `evo2` package (and
torch 2.5+), which is incompatible with this env's scGPT-pinned torch 2.1.2.
Evo-1 (togethercomputer/evo-1-8k-base) is HF-native via trust_remote_code,
same 7B / d_model=4096 / 32-layer StripedHyena family, and runs cleanly here
once flash_attn + einops are installed.

Flat script per the phase 7 design (no bio_fm_probe toolkit). The whole flow
in one file:

  1. Load Evo-1 via HF AutoModelForCausalLM(trust_remote_code).
...
Output dir: results/evo_1_8k__genomic_benchmarks/ (default; --out overrides).

Usage:
    python src/phase7/evo_probe.py
    python src/phase7/evo_probe.py --sae_expansion 32 --sae_layers layer_00_input layer_16 layer_32
    python src/phase7/evo_probe.py --max_samples 5000 --force   # cheap dry run
"""
...
# ============================================================================
# GenomicBenchmarks human_nontata_promoters sequences are ~250 nt; 512 covers
# all of them with margin AND keeps StripedHyena's FFT (parallel_iir) memory
# bounded — at 8192 with batch_size=4, the Hyena filter intermediate alone
# overflows a 48 GB A6000.
MAX_LEN_DEFAULT = 512
...
def load_evo2(model_dir: str, device: str = "cuda", fp16: bool = True):
    ...
    # appears in real sequences and the attention_mask will correctly flag it.
    if tok.pad_token is None:
        tok.pad_token = chr(0)
    # Evo-1's auto_map registers only AutoModelForCausalLM (no bare AutoModel).
    ...
    if blocks_attr is None:
        raise SystemExit(
            "[evo] couldn't locate the block list on the model. Check "
    ...
    if d_model is None:
        for p in model.parameters():
            if p.dim() == 2:
                d_model = p.shape[-1]
                break
    print(f"[evo] loaded: n_layers={n_layers}, d_model={d_model}, "
...
def main() -> int:
    ...
    p.add_argument("--extract_batch_size", type=int, default=2,
                   help="StripedHyena's FFT filter is memory-hungry; bs=2 at "
                        "max_len=512 peaks ~32 GB on A6000. bs=4 OOMs.")
    ...
    p.add_argument("--out", default=None,
                   help="default: results/evo_1_8k__genomic_benchmarks/")
    args = p.parse_args()
    ...
    if len(seqs) > args.max_samples:
        ...
        labels = labels[idx]
        print(f"[evo] subsampled to {len(seqs)}")
    ...
    print(f"[evo] tokenizing (max_len={args.max_len}) ...")
    encoded = tok(
    ...
    print(f"[evo] tokenized: input_ids={input_ids.shape}")
    ...
    for layer in sae_layers:
        ...
        if acts_cache.exists() and not args.force:
            ...
            print(f"[evo] mmap-loading cached activations from {acts_cache} ({sz_gb:.2f} GB)")
            # mmap_mode='r' = instant load, chunks read from disk on demand
            d = np.load(acts_cache, allow_pickle=False, mmap_mode="r")
            token_acts, token_cell = d["acts"], d["cell_idx"]
            print(f"[evo] cached shapes: acts={token_acts.shape} {token_acts.dtype}, "
            ...
            print(f"[evo] extracted {token_acts.shape} in {time.time() - t0:.0f}s")

        n_cells = len(seqs)

        # ---- SVD diagnostic ----
        svd_results[layer] = svd_spectrum_diagnostic(token_acts)
        print(f"[evo] SVD: PR={svd_results[layer]['participation_ratio']:.1f}, "
              f"k95={svd_results[layer]['k95']}, k99={svd_results[layer]['k99']}")

        # ---- PCA fit + per-sample features ----
        # Memory-safe path: subsample-fit, then chunked-project-and-aggregate so
        # we never materialize the (n_tokens, n_components) fp64 matrix
        # (naive sklearn fit_transform on 5M×4096 needs ~328 GB working RAM).
        print(f"[evo] fitting PCA(n={pca_dim}) on {token_acts.shape[0]} tokens "
              f"(streaming, chunked) ...")
        ...
        if sae_ckpt.exists() and not args.force:
            print(f"[evo] loading cached SAE from {sae_ckpt}")
        ...
        print(f"[evo] encoding SAE features ...")
        ...
        print(f"[evo]   PCA probe acc = {pca_probe['accuracy_mean']:.4f} "
              f"± {pca_probe['accuracy_std']:.4f}")
        print(f"[evo]   SAE probe acc = {sae_probe['accuracy_mean']:.4f} "
        ...
        if not K_grid:
            K_grid = [1, 2, 4, 8]
        print(f"[evo]   ablation K-grid (truncated at k99={k99}): {K_grid}")
        ...
        print(f"[evo]   any K significant (2σ above null) = "
